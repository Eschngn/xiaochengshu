package com.swc.orangeBook.user.relation.biz.service.impl;
...
import com.swc.orangeBook.user.relation.biz.constant.MQConstants;
import com.swc.orangeBook.user.relation.biz.constant.RedisKeyConstants;
import com.swc.orangeBook.user.relation.biz.domain.dataobject.FansDO;
import com.swc.orangeBook.user.relation.biz.domain.dataobject.FollowingDO;
import com.swc.orangeBook.user.relation.biz.domain.mapper.FansDOMapper;
import com.swc.orangeBook.user.relation.biz.domain.mapper.FollowingDOMapper;
import com.swc.orangeBook.user.relation.biz.enums.LuaResultEnum;
import com.swc.orangeBook.user.relation.biz.enums.ResponseCodeEnum;
import com.swc.orangeBook.user.relation.biz.model.dto.FollowUserMqDTO;
import com.swc.orangeBook.user.relation.biz.model.dto.UnfollowUserMqDTO;
import com.swc.orangeBook.user.relation.biz.model.vo.*;
import com.swc.orangeBook.user.relation.biz.rpc.UserRpcService;
import com.swc.orangeBook.user.relation.biz.service.RelationService;
...
public class RelationServiceImpl implements RelationService {
    ...
    public Response<?> follow(FollowUserReqVO followUserReqVO) {
        ...
        // 当前时间转时间戳
        long timestamp = DateUtil.localDateTime2Timestamp(now);
        // 执行 Lua 脚本，拿到返回结果
    ...
    private static Object[] buildLuaArgs(List<FollowingDO> followingDOS,long expireSeconds){
        ...
        for (FollowingDO followingDO : followingDOS) {
            luaArgs[i]=DateUtil.localDateTime2Timestamp(followingDO.getCreateTime()); // 关注时间作为 score
            luaArgs[i+1]=followingDO.getFollowingUserId();  // 关注的用户 ID 作为 ZSet value
            i+=2;
        }
        luaArgs[argsLength-1]=expireSeconds;
        return luaArgs;// 最后一个参数是 ZSet 的过期时间
    }
    ...
    private static Object[] buildFansZSetLuaArgs(List<FansDO> fansDOS, long expireSeconds) {
        ...
        for (FansDO fansDO : fansDOS) {
            luaArgs[i] = DateUtil.localDateTime2Timestamp(fansDO.getCreateTime()); // 粉丝的关注时间作为 score
