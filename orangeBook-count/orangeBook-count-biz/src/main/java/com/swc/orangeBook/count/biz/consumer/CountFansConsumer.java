 ...
 * @author Wilson
 * @Description: 计数: 粉丝数
 * @date 2024/10/11 1:05
 */
@Component
@RocketMQMessageListener(consumerGroup = "xiaochengshu_group_" + MQConstants.TOPIC_COUNT_FANS, // Group 组
...
public class CountFansConsumer implements RocketMQListener<String> {
    ...
    private void consumeMessage(List<String> bodys) {
        ...
        List<CountFollowUnfollowMqDTO> countFollowUnfollowMqDTOS = bodys.stream()
                .map(body -> JsonUtil.parseObject(body, CountFollowUnfollowMqDTO.class)).toList();
        ...
        for (Map.Entry<Long, List<CountFollowUnfollowMqDTO>> entry : groupMap.entrySet()) { // 遍历所有的键值对
